<template>
  <el-card class="main-card">
    <div class="title" v-if="this.$route.name === '修改文章'">
      修改餐厅
    </div>
    ...
    <div class="article-title-container">
      <el-input v-model="article.articleTitle" size="medium" placeholder="输入标题"/>

      <el-button type="danger" size="medium" @click="openModel" style="margin-left: 10px"> 发布</el-button>
    </div>
...
<!--      <div class="dialog-title-container" slot="title">发布</div>-->
      <div class="dialog-title-container" slot="title">发布</div>
...
<!--        <el-form-item label="分类">-->
        ...
        <el-form-item label="访问密码" v-if="article.status == 2">
          <el-input v-model="article.password" placeholder="请填写访问密码"/>
        </el-form-item>
      </el-form>
      <div slot="footer">
      ...
      this.axios.post('/api/admin/articles', this.article).then(({data}) => {
        if (data.flag) {
          if (this.article.id === null) {
            this.$store.commit('removeTab', '发布')
          } else {
            this.$store.commit('removeTab', '修改')
          }
          sessionStorage.removeItem('article')
          // this.$router.push({path: '/article-list'})
          this.$router.push({path: '/resta-list'})
          this.$notify.success({
            title: '成功',
            message: '保存草稿成功'
          })
